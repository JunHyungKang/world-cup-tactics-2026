...
Status: `LOCAL STATIC RELEASE REHEARSAL PASS — PROMOTE 98/100`
...
The shortcut removes judging friction without merging the two holdouts.

| Time | Screen action | Korean narration | Observable proof |
|---|---|---|---|
| 0–5s | Hold on the `48 → 8 → 8` campaign map | `조별리그에서 세우고, 토너먼트에서 깨뜨립니다.` | Reference, rehearsal, and sealed final-audit partitions are visible |
...
| 45–59.5s | Hold the final receipt | `정책 변경은 없습니다. 위치 겹침만 세며, ‘막았을 것이다’ 또는 ‘최적 정책이다’라는 추론은 하지 않습니다.` | `정책 변경 0회`, same fingerprint, claim boundary |
...
- Video SHA-256: `558e8f0b02aab094e2eb366acfa8becf50527f9f5f16ebea96a8fd30e586e22f`
- Captions: Korean captions are burned into the rehearsal and retained as a byte-bound SRT sidecar
- Timed path: `7/7` timed events within 0.1 seconds, five activations, one policy
  lock, one explicit scroll, and final receipt visible at `34.005s`
- Re-record requirement: regenerate the exact local artifact after every Quick
  Trial code or copy change
- Narration: 7/7 cues fit their allocated intervals
- Independent comparison: Policy Lab `98/100`, Corner War Room `97/100`; PROMOTE
